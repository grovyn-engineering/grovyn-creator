 ...
 * The session probe.
 *
 * `/auth/me` answers 200 with `user: null` when signed out rather than 401, so
 * being logged out is an ordinary state rather than an error — which keeps the
 * login page from logging a failure on every visit and keeps this hook's
...
export function useSignup() {
  ...
  return useMutation({
    mutationFn: (input: SignupRequest) => api.auth.signup(input),
    onSuccess: (data) => {
      queryClient.setQueryData(queryKeys.session, { user: data.user });
      // Signup provisions a workspace transactionally, so a new user always
...
export function useLogout() {
  ...
  return useMutation({
    ...
    // to leave, and stranding them in a signed-in shell with a dead session is
    // worse than clearing optimistically.
    onSettled: () => {
      queryClient.clear();
      router.push("/login");
      router.refresh();
