...
import { api, errorMessage, toFormErrors } from "@/lib/api-client";
...
function ProfileCard() {
  ...
  const update = useMutation({
    mutationFn: (input: UpdateProfileRequest) => api.patch("/api/auth/profile", input),
    onSuccess: async () => {
...
function WorkspaceCard() {
  ...
  const update = useMutation({
    mutationFn: (input: UpdateWorkspaceRequest) =>
      api.patch<{ workspace: WorkspaceMembership }>("/api/workspaces/current", input, {
        workspaceId,
      }),
    onSuccess: async () => {
...
function PasswordCard() {
  ...
  const change = useMutation({
    mutationFn: (input: ChangePasswordRequest) => api.post("/api/auth/password", input),
    onSuccess: () => {
      reset();
      setSaved(true);
