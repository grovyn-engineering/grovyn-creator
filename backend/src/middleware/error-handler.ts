...
export const errorHandler: ErrorRequestHandler = (err, req, res, _next) => {
  ...
  if (!isProduction && appError.options.internal) {
    (body as ApiError & { debug?: string }).debug = describeInternal(appError.options.internal);
  }
...
/**
 * Renders the internal detail for the development-only `debug` field.
 *
 * `String(value)` is not enough: for a plain object it produces
 * "[object Object]", which is exactly the case a developer most wants to read.
 * Errors give up their stack, everything else is serialised, and an object with
 * a circular reference falls back rather than throwing inside the error handler
 * — which would turn a handled error into an unhandled one.
 */
function describeInternal(value: unknown): string {
